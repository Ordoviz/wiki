*This guide is meant for SuperTux v0.6.0. For older versions: [0.5.1](https://github.com/Ordoviz/wiki/blob/master/Editor-Guide-0-5-1.md)*

# Your first Level

Click on *Level Editor* from the title menu. Then *New level subset*. Give it a name and click *OK*. To make your first level click on *Create level* and for the sake of free software we will release our level under [CC-BY-SA](https://creativecommons.org/licenses/by-sa/4.0/).
...
Right click on the music note in the bottom bar. There is great track list to choose from. I will help you choose:
...
*.music files define where to loop the given soundtrack. Use them where available to ensure music sounds nice.

#### The Background

![](images/background.png) Right click this icon in the bottom panel. You can change the top, middle and bottom of the background separately, which allows you to mix certain images. Since this is also a complete mess, I suggest you to open the SuperTux data folder with your file manager and pick your images there.
...
- The bucket fill tool ![](images/select-mode2.png)lets you fill an area of same tiles framed by blocks of another tile.
- If something goes wrong you can undo/redo using the arrows on the left: ![](images/undo.png)![](images/redo.png)
...
Right click the info block to add a message. You can use the first character of a paragraph for formatting instructions. This character will not be displayed in the message:
...
- For normal text you can also use a hash (#) which is needed for compatibility with older levels.
...
When you decrease the size of a tile map and enable the *Following path* property, you can move the tile map at the adjustment handle at the top-left corner around (press <kbd>f9</kbd> to hide the annoying scroller) and let it move like a moving platform! See the section above how to do this. You can get inspiration for this in the level *Crystal Mine* by Carsten Wirtz, which uses this mechanic.

# Working with Lightmaps

Before lanterns and magic blocks can reasonably used, we have to darken the sector first. Click on the light bulb ![](images/ambient_light.png) in the bottom bar and define the color using RGBA. Give each value a number between 0 and 1. I do not think that you ever have to change the alpha value, leave it 1.
...
Before you can test your worldmap, you have to activate it. Go back again to *Level subset properties* and change *Type* to *Worldmap*. After this you can test your worldmap from the title menu in *Start Game* > *Contrib Levels*.
...
```
Define background and music. Try for example `"nightsky_top.png"` and `"music/voc-dark.music"`. The text can be formatted just like the text in info blocks.
...
Navigate to *Objects* > *Ambient*, select the clock icon ![](images/clock.png) and add it. The clock icon now appears in the bottom bar. Right click on it and enter the desired time.

# Tips and additional Resources

- You can learn a lot by looking at other people’s levels in the editor. Set *hide-from-contribs* in the SuperTux data folder to `#f` to view the story mode levels.

  - Linux: `/usr/share/games/supertux2/levels/world1/info` and `/usr/share/games/supertux2/levels/world2/info` 
  - Windows: `C:\Program Files\SuperTux\data\levels\world1\info` and `C:\Program Files\SuperTux\data\levels\world2\info`
